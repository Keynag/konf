...
git clone https://aur.archlinux.org/xpadneo-dkms.git && gpg --import ~/konf/arch/system/anonfunc.asc;

sudo pacman-key --init && sudo pacman-key --populate;

sudo pacman -S llvm clang lld mold openmp --needed;

sudo cp -r ~/konf/arch/system/makepkg.conf /etc/makepkg.conf;
...
sudo cp -r ~/konf/arch/system/pacman.conf /etc/pacman.conf;

sudo cp -r ~/konf/arch/system/environment /etc/environment;

sudo cp -r ~/konf/arch/system/geoclue.conf /etc/geoclue/geoclue.conf;
